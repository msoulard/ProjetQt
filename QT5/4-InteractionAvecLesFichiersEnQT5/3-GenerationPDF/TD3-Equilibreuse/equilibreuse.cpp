...
{
    QString nomFichier;
    QFileDialog fichierDial(this);
    QString cheminFichier;
    fichierDial.setAcceptMode(QFileDialog::AcceptSave);
    fichierDial.setWindowTitle("Enregistrment du fichier PDF");
    fichierDial.setViewMode(QFileDialog::Detail);
    fichierDial.setOption(QFileDialog::DontResolveSymlinks);
    fichierDial.setNameFilters(QStringList()<<"PDF File (*.pdf)");
    fichierDial.setDefaultSuffix("pdf");
    if(fichierDial.exec() && fichierDial.selectedFiles().size() != 0){
        cheminFichier = fichierDial.selectedFiles()[0];
        //création du PDF
        RapportMesures rapport(*experience, *chartView, nomFichier);
...
{
    DialogConfigurerExperience dialConf(experience, this);
    dialConf.exec();
}
